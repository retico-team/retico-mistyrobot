...
class MistyRandomMoves:
    #functions taken from mistyPy.py for testing
    def move_arm(self,arm,position,velocity=100):
            if position <= -91:# " moveArm: position needs to be -90 to 90"
                position = -91
            if position >= 91:
                position = 91
    ...
    def move_head(self,roll,pitch,yaw,velocity=100):
        assert -45 <= roll <= 45
        assert -45 <= pitch <= 45
        assert -70 <= yaw <= 70
    ...
    def begin_random_movements(self):
        while True:
            return_to_middle = random.randrange(0, 100, 2)
            if return_to_middle > 25:
                if bool(random.getrandbits(1)): self.roll += random.randrange(-2, 2, 2)
                if bool(random.getrandbits(1)): self.pitch += random.randrange(-4, 4, 2)
                if bool(random.getrandbits(1)): self.yaw = +random.randrange(-5, 5, 2)
                self.move_head(self.roll,self.pitch,self.yaw)
        
                if bool(random.getrandbits(1)): self.arm += random.randrange(-15, 5, 2) 
                if bool(random.getrandbits(1)): self.move_arm('right',self.arm)
                if bool(random.getrandbits(1)): self.move_arm('left',self.arm)
                time.sleep(random.randrange(0, 100, 2) * 0.01)
            else:
                self.reset()
                # self.move_head(0,0,0)
                self.move_arm('left',80)
                self.move_arm('right',80)
                time.sleep(2.0)

    # todo: set central starting/returning point
...
class Robot:
    ...
    def start_small_random_movements(self):
        ...
        def inner():
            mr = MistyRandomMoves(self.ip)
            mr.begin_random_movements()
...
#     robot.say("ahoy there")
#     robot.move_arm('left', -50)
    # robot.takePicture()
    # robot.takePicture()